import React, { useState, useMemo, useRef, useCallback, forwardRef, useImperativeHandle, memo, useLayoutEffect, useEffect } from 'react';
...
import { useInteractivePanGestureHandler, useScrollable, usePropsValidator, useNormalizedSnapPoints, useReactiveSharedValue, useKeyboard } from '../../hooks';
...
import { GESTURE, ANIMATION_STATE, WINDOW_HEIGHT, KEYBOARD_STATE, KEYBOARD_BEHAVIOR, SHEET_STATE, SCROLLABLE_STATE, KEYBOARD_BLUR_BEHAVIOR } from '../../constants';
import { animate, getKeyboardAnimationConfigs } from '../../utilities';
import { DEFAULT_ANIMATION_EASING, DEFAULT_ANIMATION_DURATION, DEFAULT_HANDLE_HEIGHT, DEFAULT_OVER_DRAG_RESISTANCE_FACTOR, DEFAULT_ENABLE_CONTENT_PANNING_GESTURE, DEFAULT_ENABLE_HANDLE_PANNING_GESTURE, DEFAULT_ENABLE_OVER_DRAG, DEFAULT_ENABLE_FLASH_SCROLLABLE_INDICATOR_ON_EXPAND, DEFAULT_ANIMATE_ON_MOUNT, DEFAULT_KEYBOARD_BEHAVIOR, DEFAULT_KEYBOARD_BLUR_BEHAVIOR } from './constants';
import { styles } from './styles';
Animated.addWhitelistedUIProps({
  decelerationRate: true
});
const BottomSheetComponent = /*#__PURE__*/forwardRef((props, ref) => {
  ...
  const [containerHeight, setContainerHeight] = useState(_providedContainerHeight);
  const [handleHeight, setHandleHeight] = useState(_providedHandleHeight); // safe layout values

  const safeHandleHeight = useMemo(() => handleComponent === null ? 0 : handleHeight || DEFAULT_HANDLE_HEIGHT, [handleHeight, handleComponent]);
  const safeContainerHeight = useMemo(() => _providedContainerHeight || containerHeight || WINDOW_HEIGHT, [_providedContainerHeight, containerHeight]); // conditions

  const shouldMeasureContainerHeight = useMemo(() => _providedContainerHeight === undefined, [_providedContainerHeight]);
  const shouldMeasureHandleHeight = useMemo(() => _providedHandleHeight === undefined && handleComponent !== undefined && handleComponent !== null, [_providedHandleHeight, handleComponent]); // refs
  ...
  const currentIndexRef = useRef(animateOnMount ? -1 : _providedIndex);
  const isClosing = useRef(false);
  const didMountOnAnimate = useRef(false); // scrollable variables
  ...
  } = useKeyboard(); // normalize snap points

  const snapPoints = useNormalizedSnapPoints(_providedSnapPoints, topInset, safeContainerHeight, safeHandleHeight);
  const sheetHeight = useMemo(() => safeContainerHeight - snapPoints[snapPoints.length - 1] - safeHandleHeight, [snapPoints, safeContainerHeight, safeHandleHeight]);
  ...
  const handleOnAnimate = useCallback(toPoint => {
  ...
  const handleSettingScrollableRef = useCallback(scrollableRef => {
  ...
  const animatedSheetHeight = useDerivedValue(() => {
    if (keyboardBehavior === KEYBOARD_BEHAVIOR.none || keyboardBehavior === KEYBOARD_BEHAVIOR.extend) {
      return sheetHeight;
    }
    ...
    if (keyboardBehavior === KEYBOARD_BEHAVIOR.interactive && isExtendedByKeyboard.value) {
  ...
  const animatedIndex = useDerivedValue(() => {
    ...
    return isLayoutCalculated ? interpolate(animatedPosition.value, adjustedSnapPoints, adjustedSnapPointsIndexes, Extrapolate.CLAMP) : -1;
  ...
  const animatedSheetState = useDerivedValue(() => {
    ...
    } else if (animatedPosition.value === extendedSheetPosition || keyboardBehavior === KEYBOARD_BEHAVIOR.interactive && isExtendedByKeyboard.value && animatedPosition.value === extendedSheetWithKeyboardPosition) {
  ...
  const animatedCurrentIndex = useSharedValue(currentIndexRef.current); // callbacks

  const animateToPointCompleted = useWorkletCallback(() => {
    animationState.value = ANIMATION_STATE.STOPPED;
  });
  const animateToPoint = useWorkletCallback((point, velocity = 0, configs) => {
    /**
    ...
    if (configs !== undefined) {
      animatedPosition.value = animate(point, configs, velocity, animateToPointCompleted);
    } else if (_providedAnimationConfigs) {
      /**
       * use animationConfigs callback, if provided
       */
      animatedPosition.value = animate(point, _providedAnimationConfigs, velocity, animateToPointCompleted);
    } else {
      animatedPosition.value = animate(point, {
  ...
  const scrollableState = useDerivedValue(() => {
    return animatedSheetState.value === SHEET_STATE.FULL_SCREEN || animatedSheetState.value === SHEET_STATE.EXTENDED ? SCROLLABLE_STATE.UNLOCKED : SCROLLABLE_STATE.LOCKED;
  }); // hooks

  const [contentPanGestureHandler, contentPanGestureState] = useInteractivePanGestureHandler({
  ...
  const [handlePanGestureHandler, handlePanGestureState] = useInteractivePanGestureHandler({
    type: GESTURE.HANDLE,
    enableOverDrag,
    overDragResistanceFactor,
    keyboardState,
  ...
  const handleOnContainerMeasureHeight = useCallback(height => {
    // console.log('BottomSheet', 'handleOnContainerMeasureHeight', height);
    didSetContainerHeight.current = true;
    setContainerHeight(height);
  }, []);
  const handleOnHandleMeasureHeight = useCallback(height => {
  ...
  const handleSnapTo = useCallback((index, animationDuration = DEFAULT_ANIMATION_DURATION, animationEasing = DEFAULT_ANIMATION_EASING) => {
  ...
  const handleClose = useCallback((animationDuration = DEFAULT_ANIMATION_DURATION, animationEasing = DEFAULT_ANIMATION_EASING) => {
    /**
     * verify if sheet is closed.
     */
    if (animatedPosition.value === safeContainerHeight) {
  ...
  const handleExpand = useCallback((animationDuration = DEFAULT_ANIMATION_DURATION, animationEasing = DEFAULT_ANIMATION_EASING) => {
    /**
     * verify if sheet is closed.
     */
    if (animatedPosition.value === safeContainerHeight) {
  ...
  const handleCollapse = useCallback((animationDuration = DEFAULT_ANIMATION_DURATION, animationEasing = DEFAULT_ANIMATION_EASING) => {
    /**
     * verify if sheet is closed.
     */
    if (animatedPosition.value === safeContainerHeight) {
  ...
  const contentMaskContainerStyle = useMemo(() => ({ ...styles.contentMaskContainer,
  ...
  useAnimatedReaction(() => keyboardState.value, (state, previousState) => {
    ...
    if (keyboardBlurBehavior === KEYBOARD_BLUR_BEHAVIOR.restore && state === KEYBOARD_STATE.HIDDEN && contentPanGestureState.value !== State.ACTIVE && handlePanGestureState.value !== State.ACTIVE) {
    ...
    if (keyboardBehavior === KEYBOARD_BEHAVIOR.extend && state === KEYBOARD_STATE.SHOWN) {
      const newSnapPoint = snapPoints[snapPoints.length - 1];
      animateToPoint(newSnapPoint, 0, animationConfigs);
      return;
    }
    ...
    if (keyboardBehavior === KEYBOARD_BEHAVIOR.fullScreen && state === KEYBOARD_STATE.SHOWN) {
    ...
    if (keyboardBehavior === KEYBOARD_BEHAVIOR.interactive && state === KEYBOARD_STATE.SHOWN) {
      isExtendedByKeyboard.value = true;
      const newSnapPoint = snapPoints[snapPoints.length - 1];
  ...
  useAnimatedReaction(() => animatedPosition.value, value => {
  ...
  useAnimatedReaction(() => animatedIndex.value, value => {
  ...
  useAnimatedReaction(() => ({
    ...
    if (_animatedIndex % 1 === 0 && _animationState === ANIMATION_STATE.STOPPED && (_contentGestureState === State.END || _contentGestureState === State.UNDETERMINED) && (_handleGestureState === State.END || _handleGestureState === State.UNDETERMINED)) {
  ...
  }, isLayoutCalculated && /*#__PURE__*/React.createElement(BottomSheetDraggableView, {
    key: "BottomSheetRootDraggableView",
    style: contentContainerStyle
  }, typeof children === 'function' ? children() : children)), /*#__PURE__*/React.createElement(BottomSheetHandleContainer, {
